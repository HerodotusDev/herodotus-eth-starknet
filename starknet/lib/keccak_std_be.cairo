...
func keccak256_auto_finalize{range_check_ptr, keccak_ptr: felt*, bitwise_ptr: BitwiseBuiltin*}(
    input: IntsSequence
) -> (output: felt*) {
    alloc_locals;
...
func keccak256{range_check_ptr, keccak_ptr: felt*, bitwise_ptr: BitwiseBuiltin*}(
    ...
    let (keccak_hash) = keccak_as_words{keccak_ptr=keccak_ptr}(
        input_le.element, input_le.element_size_bytes
    );
...
func to_le{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(input: IntsSequence) -> (
    res: IntsSequence
) {
    alloc_locals;
...
func to_le_rec{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
    input: IntsSequence, acc_len: felt, acc: felt*
) {
    alloc_locals;
