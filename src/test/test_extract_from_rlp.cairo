...
from starknet.lib.extract_from_rlp import (
    extract_data,
    is_rlp_list,
    to_list,
    getElement,
...
//     let (is_list) = is_rlp_list(0, input);
//     assert is_list = 1;
//     return ();
// }
...
//     let (values: felt*) = alloc();
//     %{
//         segments.write_arg(ids.values, [0x82beef])
//     %}
//     local input: IntsSequence = IntsSequence(values, 3, 3);
...
func test_to_list{range_check_ptr}() -> () {
    ...
    let (list_len, data_positions, list_len, lengths) = helper_test_to_list(block_rlp_len, block_rlp_len, values);
    ...
    %{
        ...
        # TODO: find a way to dynamically assert the following:
        # assert ids.lengths == expected_lengths[0]
        # assert ids.data_positions == expected_data_positions
    %}
    return ();
}

// @view
// func test_extractData{range_check_ptr}(
...
// @view
// func test_get_element{range_check_ptr}(
//     rlp_len_bytes: felt, rlp_len: felt, rlp: felt*, position: felt
// ) -> (res: RLPItem) {
//     alloc_locals;
//     local input: IntsSequence = IntsSequence(rlp, rlp_len, rlp_len_bytes);
