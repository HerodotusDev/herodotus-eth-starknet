...
func verify_l2_output_root_bedrock{
    ...
    let (local receipt_rlp_items: RLPItem*, receipt_rlp_items_len: felt) = to_list(
        valid_receipt_rlp
    );
    ...
    let (local output_root_preimage_hash: Keccak256Hash) = calculate_output_root_preimage_hash(
    ...
    IEthereumHeadersStore.receive_from_l1(
        optimism_headers_store, 4, block_hash_words, l2_block_number + 1
    );
    return ();
}
...
func calculate_output_root_preimage_hash{
    ...
    local res: Keccak256Hash = Keccak256Hash(
        preimage_hash[0], preimage_hash[1], preimage_hash[2], preimage_hash[3]
    );
    return (res,);
}
...
func decode_l2_output_root_from_log_topic{
    ...
    local res: Keccak256Hash = Keccak256Hash(
        data.element[0], data.element[1], data.element[2], data.element[3]
    );
    return (res,);
}

func decode_l2_output_index_from_log_topic{range_check_ptr}(topic: IntsSequence) -> (
    output_index: felt
) {
    alloc_locals;
...
func decode_l2_block_number_from_log_topic{range_check_ptr}(topic: IntsSequence) -> (
    output_index: felt
) {
    alloc_locals;
...
func access_l1_header_from_mmr{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, bitwise_ptr: BitwiseBuiltin*, range_check_ptr
}(
    mmr_inclusion_header_leaf_index: felt,
    mmr_inclusion_header_leaf_value: felt,
    ...
    let (mmr_root) = IEthereumHeadersStore.get_tree_size_to_root(
        contract_address=headers_store_addr, tree_size=mmr_inclusion_header_pos
    );
    assert_not_zero(mmr_root);

    // Verify the header inclusion in the headers store's MMR.
    IEthereumHeadersStore.call_mmr_verify_proof(
...
func verify_receipt_proof_against_header{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, bitwise_ptr: BitwiseBuiltin*, range_check_ptr
}(
    block_header: IntsSequence,
    path_size_bytes: felt,
...
func assert_tx_succeed{
    syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, bitwise_ptr: BitwiseBuiltin*, range_check_ptr
}(receipt_rlp_items: RLPItem*, receipt_rlp_items_len: felt, receipt_rlp: IntsSequence) {
    alloc_locals;
